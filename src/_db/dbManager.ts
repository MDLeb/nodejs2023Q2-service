...
class dbManager {
  ...
  isUniqLogin(userLogin: string): boolean {
    return this.#dbUsers.isUniqLogin(userLogin);
  }
  ...
  updateUser(id: string, userdata: Partial<IUserData>): User | null {
    const user = JSON.parse(
      JSON.stringify(this.#dbUsers.changeUser(id, userdata)),
    );
    delete user.password;
    return user;
  }
  ...
  getAllFavs(): IFavorites {
    ...
    return {
      artists: res.artists
        .map((j) => this.#dbArtists.getArtistById(j))
        .filter((i) => i !== null),
      albums: res.albums
        .map((j) => this.#dbAlbums.getAlbumById(j))
        .filter((i) => i !== null),
      tracks: res.tracks
        .map((j) => this.#dbTracks.getTrackById(j))
        .filter((i) => i !== null),
    };
  }
  ...
  isItemUniq(id: string): boolean {
    return this.#dbFavorites.isItemUniq(id);
  }
}
